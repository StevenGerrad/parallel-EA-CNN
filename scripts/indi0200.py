...
class EvoCNNModel(nn.Module):
    def __init__(self):
        ...
        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(8),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.Linear(19, 10)))
        ...
        self.layer_edge.append(torch.nn.Conv2d(3,6,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))
    ...
    def forward(self, input):
        ...
        e4 = self.layer_edge[4](x1)

        e1 = x2

        e2 = self.layer_edge[2](x0)

        x3 = torch.cat([e4,e1,e2], dim=1)
