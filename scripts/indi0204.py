...
class EvoCNNModel(nn.Module):
    def __init__(self):
        ...
        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(4),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(11),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.Linear(14, 10)))
        ...
        self.layer_edge.append(torch.nn.Conv2d(4,11,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))

        self.layer_edge.append(None)

        self.layer_edge.append(torch.nn.Conv2d(3,4,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))
    ...
    def forward(self, input):
        ...
        x0 = input
        x1 = x0
        x2 = self.layer_edge[4](x1)
        x2 = self.layer_vertex[2](x2)

        x3 = self.layer_edge[2](x2)
