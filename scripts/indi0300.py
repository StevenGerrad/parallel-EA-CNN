...
class EvoCNNModel(nn.Module):
    def __init__(self):
        ...
        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(14),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(49),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.Linear(57, 10)))
        ...
        self.layer_edge.append(torch.nn.Conv2d(14,38,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))
        ...
        self.layer_edge.append(torch.nn.Conv2d(3,8,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))
    ...
    def forward(self, input):
        ...
        e6 = self.layer_edge[6](x1)

        x2 = torch.cat([e7,e6], dim=1)
        x2 = self.layer_vertex[2](x2)

        e3 = self.layer_edge[3](x2)

        e4 = self.layer_edge[4](x0)

        e8 = self.layer_edge[8](x1)

        x3 = torch.cat([e3,e4,e8], dim=1)
        ...
        x4 = torch.cat([e2,e5,e1], dim=1)
        x4 = self.globalPool(x4)

        x4 = torch.squeeze(x4, 3)

        x4 = torch.squeeze(x4, 2)
