...
class EvoCNNModel(nn.Module):
    def __init__(self):
        ...
        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(12),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.Linear(21, 10)))
        ...
        self.layer_edge.append(torch.nn.Conv2d(3,9,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))
    ...
    def forward(self, input):
        ...
        x0 = input
        x1 = x0
        e2 = self.layer_edge[2](x1)

        e4 = self.layer_edge[4](x0)

        x2 = torch.cat([e2,e4], dim=1)
        x2 = self.layer_vertex[2](x2)

        e1 = x2

        e3 = self.layer_edge[3](x1)

        x3 = torch.cat([e1,e3], dim=1)
