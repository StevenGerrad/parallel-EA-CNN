...
class EvoCNNModel(nn.Module):
    def __init__(self):
        ...
        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(3),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(16),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(47),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.Linear(55, 10)))
        ...
        self.layer_edge.append(torch.nn.Conv2d(16,44,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))
    ...
    def forward(self, input):
        ...
        x0 = input
        x1 = x0
        x2 = self.layer_edge[8](x1)
        x2 = self.layer_vertex[2](x2)

        e7 = self.layer_edge[7](x0)

        e6 = self.layer_edge[6](x2)

        x3 = torch.cat([e7,e6], dim=1)
        ...
        x4 = torch.cat([e3,e4], dim=1)
        x4 = self.layer_vertex[4](x4)

        e5 = self.layer_edge[5](x1)

        e2 = self.layer_edge[2](x0)

        e1 = x4

        x5 = torch.cat([e5,e2,e1], dim=1)
