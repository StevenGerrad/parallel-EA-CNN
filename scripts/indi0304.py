...
class EvoCNNModel(nn.Module):
    def __init__(self):
        ...
        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(11),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(46),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.BatchNorm2d(144),torch.nn.ReLU(inplace=True)))

        self.layer_vertex.append(torch.nn.Sequential(torch.nn.Linear(149, 10)))
        ...
        self.layer_edge.append(torch.nn.Conv2d(46,127,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))

        self.layer_edge.append(torch.nn.Conv2d(3,3,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))

        self.layer_edge.append(torch.nn.Conv2d(3,11,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))

        self.layer_edge.append(None)

        self.layer_edge.append(torch.nn.Conv2d(11,43,kernel_size=(3,3),stride=pow(2, 0),padding=(1, 1)))
    ...
    def forward(self, input):
        ...
        x0 = input
        x1 = x0
        x2 = self.layer_edge[5](x1)
        x2 = self.layer_vertex[2](x2)

        e9 = self.layer_edge[9](x1)

        e7 = self.layer_edge[7](x2)

        x3 = torch.cat([e9,e7], dim=1)
        ...
        e4 = self.layer_edge[4](x0)

        e3 = self.layer_edge[3](x3)

        x4 = torch.cat([e6,e8,e4,e3], dim=1)
        x4 = self.layer_vertex[4](x4)

        e1 = x4

        e2 = self.layer_edge[2](x0)

        x5 = torch.cat([e1,e2], dim=1)
        x5 = self.globalPool(x5)

        x5 = torch.squeeze(x5, 3)

        x5 = torch.squeeze(x5, 2)

        x5 = self.layer_vertex[5](x5)
